    ...
    <div className="min-h-screen bg-gray-950 text-white py-20 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-purple-500/10 border border-purple-500/30 rounded-full px-4 py-1 mb-6">
          ...
          <div className="flex items-center justify-center gap-4 mt-8">
            <span className={cycle === 'MONTHLY' ? 'text-white font-semibold' : 'text-gray-500'}>Monthly</span>
            <button
              onClick={() => setCycle(c => c === 'MONTHLY' ? 'YEARLY' : 'MONTHLY')}
              className={`relative w-14 h-7 rounded-full transition-colors ${cycle === 'YEARLY' ? 'bg-purple-600' : 'bg-gray-700'}`}
            >
              <motion.div
            ...
            <span className={cycle === 'YEARLY' ? 'text-white font-semibold' : 'text-gray-500'}>
              Yearly <span className="text-green-400 text-xs font-medium ml-1">Save 17%</span>
            </span>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
          className="bg-gradient-to-r from-emerald-900/30 to-teal-900/30 border border-emerald-700/30 rounded-2xl p-6 mb-12 text-center">
          <h3 className="text-lg font-semibold text-emerald-300 mb-2">Industry-Leading Low Fees</h3>
          <div className="flex flex-wrap justify-center gap-8 text-sm">
            <div><span className="text-emerald-400 font-bold text-xl">10-15%</span> <span className="text-gray-400">from freelancer</span></div>
            <div className="text-gray-600">vs Fiverr's 20%</div>
            <div><span className="text-emerald-400 font-bold text-xl">3-5%</span> <span className="text-gray-400">from client</span></div>
            <div className="text-gray-600">vs Upwork's 5-20%</div>
          </div>
        </motion.div>
          ...
          {plans.map((plan, i) => {
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-2xl overflow-hidden ${plan.tier === 'PRO' ? 'ring-2 ring-purple-500 scale-105' : ''}`}
              >
                {plan.tier === 'PRO' && (
                  <div className="absolute top-0 left-0 right-0 bg-purple-600 text-center py-1 text-xs font-bold uppercase tracking-wider">
                    Most Popular
                  </div>
                )}
                <div className={`bg-gradient-to-br ${style.gradient} p-8 ${plan.tier === 'PRO' ? 'pt-10' : ''}`}>
                ...
                <div className="bg-gray-900 p-8 border border-gray-800 rounded-b-2xl">
                  ...
                  {isActive ? (
                    <div className="space-y-2">
                      <div className="text-center text-sm text-green-400 font-medium py-2 bg-green-900/20 rounded-lg">
                        Current Plan
                      </div>
                      <button onClick={handleCancel} className="w-full text-sm text-gray-500 hover:text-red-400 transition">
                        Cancel subscription
                      </button>
                    </div>
                  ) : plan.tier === 'FREE' ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
          className="mt-16 bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-10 border border-gray-700 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-8 text-sm text-gray-400">
            {['Team seats', 'Bulk hiring', 'Custom workflows', 'SSO & API access', 'Dedicated manager', 'Monthly budget controls'].map(f => (
              <span key={f} className="flex items-center gap-2 bg-gray-800 px-3 py-1 rounded-full">
