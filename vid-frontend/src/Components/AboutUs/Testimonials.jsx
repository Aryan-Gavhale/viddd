      ...
      <section className="relative">
        ...
        <div className="relative bg-white dark:bg-slate-900 shadow-xl dark:shadow-black/40 rounded-lg overflow-hidden">
          <div className="px-4 py-8 sm:p-10">
            <h2 className="text-3xl font-bold text-gray-900 dark:text-slate-100 mb-8 text-center">What Our Users Say</h2>
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {testimonials.map((testimonial) => (
                <div key={testimonial.name} className="flex flex-col items-center text-center bg-gradient-to-b from-indigo-50 to-purple-50 dark:from-indigo-900/30 dark:to-purple-900/30 p-6 rounded-lg shadow-md">
                  <img
                    className="h-24 w-24 rounded-full object-cover mb-4 border-4 border-white dark:border-slate-700 shadow-lg"
                    src={testimonial.image || "/placeholder.svg"}
                    alt={testimonial.name}
                    width={96}
                    height={96}
                  />
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-slate-100 mb-1">{testimonial.name}</h3>
                  <p className="text-sm text-indigo-600 dark:text-indigo-300 mb-4">{testimonial.role}</p>
